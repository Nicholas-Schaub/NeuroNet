...
%% Normalize and Pad the Image
    ...
    if opts.prep.normImage
        for i = 1:size(img,3)
            I = img(:,:,i);
            img(:,:,i) = MaskedLocalResponse(I,I>0,opts.prep.windowSize);
        end
...
%% Segment the image
    ...
    for batch = 1:length(sub_batches)-1
        nn.eval({'input',img_pixels(:,:,:,sub_batches(batch)+1:sub_batches(batch+1)),'label',single(zeros(out_size,out_size,1,sub_batches(batch+1)-sub_batches(batch)))});
        if useGpu
            [~,chat] = max(gather(nn.getVar('pred').value),[],3);
            seg_img(:,:,1,sub_batches(batch)+1:sub_batches(batch+1)) = chat;
        else
            [~,chat] = max(nn.getVar('pred').value,[],3);
            seg_img(:,:,1,sub_batches(batch)+1:sub_batches(batch+1)) = chat;
        end
