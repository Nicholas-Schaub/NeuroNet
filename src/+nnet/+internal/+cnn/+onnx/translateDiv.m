function NNTLayer = translateDiv(node, LayerName, initializerDimMap, initializerRawDataMap, initializerNames, OpsetVersion)

%   Copyright 2018 The MathWorks, Inc.

% Must have exactly 2 inputs
if numel(node.input) ~= 2
    warning(message('nnet_cnn_onnx:onnx:NumNodeInputs', 'Div', 2, LayerName));
    NNTLayer = [];
    return;
end
...
if sum(isParam) ~= 1
    warning(message('nnet_cnn_onnx:onnx:OneNodeOneConst', 'Div', LayerName));
    NNTLayer = [];
    return;
end
...
% Create layer
NNTLayer = nnet.onnx.layer.ElementwiseAffineLayer(LayerName, 1./const, 0);
end
