function NNTLayer = translateSub(node, LayerName, initializerDimMap, initializerRawDataMap, initializerNames, OpsetVersion)

%   Copyright 2018 The MathWorks, Inc.

% Must have exactly 2 inputs
if numel(node.input) ~= 2
    warning(message('nnet_cnn_onnx:onnx:NumNodeInputs', 'Sub', 2, LayerName));
    NNTLayer = [];
    return;
end
...
if sum(isParam) ~= 1
    warning(message('nnet_cnn_onnx:onnx:OneNodeOneConst', 'Sub', LayerName));
    NNTLayer = [];
    return;
end
% Make a ElementwiseAffineLayer.
