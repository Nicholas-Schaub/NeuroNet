function [S] = CNNVesselSegment(img,net,opts,batchSize,useGpu)
% segment.m
%
...
%% Initialize the network
    ...
    nn.mode = 'test';
    vessel_ind = nn.getVarIndex('vessel');
    region_ind = nn.getVarIndex('region');
    if isnan(vessel_ind)
        error('Could not find prediction layer.');
    end
    nn.vars(vessel_ind).precious = 1;
    nn.vars(region_ind).precious = 1;
    if useGpu
        nn.move('gpu');
    else
        nn.move('cpu');
...
%% Segment the image
    ...
    vessel_img = single(zeros(out_size,out_size,1,sub_batches(end)));
    region_img = vessel_img;
    
    for batch = 1:length(sub_batches)-1
        nn.eval({'input',img_pixels(:,:,:,sub_batches(batch)+1:sub_batches(batch+1)),...
                 'vessel_label',single(zeros(out_size,out_size,1,sub_batches(batch+1)-sub_batches(batch))),...
                 'region_label',single(zeros(out_size,out_size,1,sub_batches(batch+1)-sub_batches(batch)))});
        if useGpu
            [~,chat] = max(gather(nn.vars(region_ind).value),[],3);
            region_img(:,:,1,sub_batches(batch)+1:sub_batches(batch+1)) = chat;
            
            vessel_img(:,:,1,sub_batches(batch)+1:sub_batches(batch+1)) = gather(nn.vars(vessel_ind).value)>0;
        else
            [~,chat] = max(nn.vars(region_ind).value,[],3);
            region_img(:,:,1,sub_batches(batch)+1:sub_batches(batch+1)) = chat;
            
            vessel_img(:,:,1,sub_batches(batch)+1:sub_batches(batch+1)) = (nn.vars(vessel_ind).value)>0;
        end
...
%% Construct segmented image
    ...
    nRows = seg_size(1);
    nCols = seg_size(2);
    S = single(zeros(seg_size(1)*out_size,seg_size(2)*out_size,2));
    img_ind = 0;
    ind = 1:out_size;
    for c = 1:nCols
        for r = 1:nRows
            img_ind = img_ind + 1;
            try
                S(ind + out_size*(r-1), ind + out_size*(c-1),1) = vessel_img(:,:,1,img_ind);
                S(ind + out_size*(r-1), ind + out_size*(c-1),2) = region_img(:,:,1,img_ind);
            catch err
                rethrow(err)
            end
        end
